...
int radialcompare(const void *a, const void *b) { /*{{{*/
    double rr = sqdist((const sh_point *)a, (const sh_point *)qsort_r_arg);
    double ss = sqdist((const sh_point *)b, (const sh_point *)qsort_r_arg);
    if (rr < ss) {
        return -1;
    }
    else if (rr > ss) {
...
sh_edge *create_edge(sh_point *p, sh_point *q, sh_triangle *t, sh_triangle *u) { /*{{{*/
    sh_edge *e = (sh_edge *) malloc(sizeof(sh_edge));
    assert(e != nullptr);
    e->p[0] = p;
    e->p[1] = q;
    e->t[0] = t;
    e->t[1] = u;
    e->flipcount = 0;
    p->neighbors.insert(q->idx);
    q->neighbors.insert(p->idx);
    return e;
} /*}}}*/
void seed_triangulation(sh_triangulation_data *td, sh_point *ps) { /*{{{*/
    sh_triangle *tri = create_triangle(&ps[0], &ps[1], &ps[2]);
    sh_edge *e[3] = {
            create_edge(&ps[2], &ps[0], tri, nullptr),
            create_edge(&ps[1], &ps[2], tri, nullptr),
            create_edge(&ps[0], &ps[1], tri, nullptr)
    };
    assert(e[0] != nullptr);
    assert(e[1] != nullptr);
    assert(e[2] != nullptr);
...
void add_point_to_hull(sh_triangulation_data *td, sh_point *p) { /*{{{*/
    ...
    sh_edge *e0 = nullptr;
    sh_edge *e1 = nullptr;
    for (ll_node *n=first_vis; n!=nullptr; n=NEXT(n)) { /*{{{*/
        ...
        e->t[1] = t;
        if (n == first_vis) {
            e0 = create_edge(e->p[0], p, t, nullptr);
        ...
        e1 = create_edge(p, e->p[1], t, nullptr);
...
void *flip_if_necessary(void *a, void *b) { /*{{{*/
    ...
    if (e->t[0] != nullptr && e->t[1] != nullptr) {
...
e->t[1]->ccr2 > sqdist(&e->t[1]->cc, e->t[0]->p[c])) {
            ...
            e->p[0]->neighbors.erase(e->p[1]->idx);
            e->p[1]->neighbors.erase(e->p[0]->idx);
            e->t[0]->p[c]->neighbors.insert(e->t[1]->p[d]->idx);
            e->t[1]->p[d]->neighbors.insert(e->t[0]->p[c]->idx);
